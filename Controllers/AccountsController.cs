...
namespace Controllers
{
    public class AccountsController : Controller
    {
        ...
        public ActionResult DeleteProfil()
        {
            ...
            //add all likes from the user
            foreach (Like like in DB.Likes.ToList())
            {
                if (like.UserID == connectedUser.Id)
                    likeIds.Add(like.Id);
            }

            
            //add all media from the user
            List<int> mediaIds = new List<int>();
            foreach (Media media in DB.Medias.ToList())
            {
                if (media.OwnerId == connectedUser.Id)
                    mediaIds.Add(media.Id);
            }

            //add all likes from deleted media to the list and delete the medias
            foreach (int mediaId in mediaIds)
            {
                foreach (Like like in DB.Likes.ToList())
                {
                    if (like.MediaID == mediaId)
                        likeIds.Add(like.Id);
                }
